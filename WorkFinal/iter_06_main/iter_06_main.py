...
def main():
    # Force parameters for Iteration 6
    # LightGBM (good with imbalance) paired with Nested RFECV
    # Note: SMOTE-ENN is now dynamically handled inside src/trainer.py CV loop.
    Config.MODEL_TYPE = 'lightgbm'
    Config.FEATURE_METHOD = 'rfecv' 
    Config.setup()
    
    print(f"\n=== Iteration 6 Started: LightGBM + SMOTE-ENN + Nested RFECV ===")
    ...
    iter_folder_name = f"iter_06_lgb_smote_rfecv_{Config.TIMESTAMP}"

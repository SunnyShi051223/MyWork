...
def main():
    Config.MODEL_TYPE = 'nam'
    Config.ENABLE_FEATURE_ENGINEERING = False
    Config.setup()
    
    print(f"\n=== Iteration 10c Started: Neural Additive Models (STRICT CV) ===")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        try:
            X_train_res, y_train_res = SMOTEENN(smote=smote, random_state=Config.SEED).fit_resample(X_train, y_train)
        except Exception as e:
            X_train_res, y_train_res = X_train, y_train
            
        print(f"       Fold {fold + 1}: Training NAM (Neural Additive Model) completely isolated on {len(X_train)} training samples...")
        
        # Model Training on strictly isolated fold
    ...
    with open(Config.GLOBAL_LOG_PATH, mode='a', newline='', encoding='utf-8') as f:
        ...
        writer.writerow([
            Config.TIMESTAMP, "nam_strict_cv",
    ...
    iter_folder_name = f"iter_10c_nam_strict_cv_{Config.TIMESTAMP}"
    backup_iteration(iter_folder_name, metrics)
    print(f"\n=== Iteration 10c Completed ===")

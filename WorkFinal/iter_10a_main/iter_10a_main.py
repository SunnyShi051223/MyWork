...
from src.deep_learning import SCLExtractor
...
def backup_iteration(iter_name, metrics):
    ...
    # Save metrics locally in backup
...
def main():
    ...
    print(f"\n=== Iteration 10a Started: SCL Tabular Embeddings (STRICT NESTED CV) ===")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        # 1. Train SCL explicitly inside the fold
        print(f"       Fold {fold + 1}: Training SCL Encoder on {len(X_train)} samples to extract 16D embeddings...")
        extractor = SCLExtractor(input_dim=X.shape[1], embed_dim=16, epochs=50, seed=Config.SEED+fold)
        extractor.fit(X_train, y_train)
        
        X_train_emb = extractor.transform(X_train)
        X_val_emb   = extractor.transform(X_val)
        
        # 2. Dynamic Resampling
        from imblearn.combine import SMOTEENN
        from imblearn.over_sampling import SMOTE
        smote = SMOTE(sampling_strategy='minority', k_neighbors=5, random_state=Config.SEED)
        try:
            X_train_res, y_train_res = SMOTEENN(smote=smote, random_state=Config.SEED).fit_resample(X_train_emb, y_train)
        except Exception as e:
            X_train_res, y_train_res = X_train_emb, y_train
    ...
    with open(Config.GLOBAL_LOG_PATH, mode='a', newline='', encoding='utf-8') as f:
        ...
        writer.writerow([
            Config.TIMESTAMP, "scl_xgboost_strict_cv",
    ...
    iter_folder_name = f"iter_10a_scl_strict_cv_{Config.TIMESTAMP}"
    backup_iteration(iter_folder_name, metrics)
    print(f"\n=== Iteration 10a Completed ===")
